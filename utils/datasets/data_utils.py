...
def h5_generator(feature_map, stage="both", train_data=None, valid_data=None, test_data=None,
    ...
    if kwargs.get("data_block_size", 0) > 0:
        from utils.datasets.data_generator import DataBlockGenerator as DataGenerator
    else:
        from utils.datasets.data_generator import DataGenerator
    ...
    if stage in ["both", "train"]:
        ...
        if 'eval_batch_size' in kwargs:
            valid_gen = DataGenerator(valid_blocks, batch_size=kwargs['eval_batch_size'], shuffle=False, **kwargs)
        else:
    ...
    if stage in ["both", "test"]:
        ...
        if len(test_blocks) > 0:
            ...
            if 'eval_batch_size' in kwargs:
                test_gen = DataGenerator(test_blocks, batch_size=kwargs['eval_batch_size'], shuffle=False, **kwargs)
            else:
